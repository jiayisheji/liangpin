$(function(){
	...
	(function(){
		...
		resetplaceholder($('#payment-money'))
		$('#payment-form').submit(function(event) {
			if($.isNumeric($('#payment-money').val()) && parseInt($('#payment-money').val()) > 1){
				alert('请保持电话畅通，稍后给您回电话');
				return true;
			}else{
				alert('最少1元，金额必须为整数');
	...
	(function(){
		...
		rollMove($('#j-partners'));
		//方案案例
		rollMove($('#j-scheme-case'));
	})();
	//筛选
	(function(){
		var $casebox = $('#j-case');
		var caseTabLi = $casebox.find('.tab').find('li');
		var caseBoxUl = $casebox.find('.case-list').find('ul');
		caseTabLi.each(function(index, el) {
			$(el).on('click',function(){
				$(this).attr('class', 'active').siblings().attr('class', '');
				caseBoxUl.eq($(this).index()).removeClass('hide').siblings().addClass('hide');
			})
		});
		select($('#filter-type'));
		var colorArr = ['red','blue','gold','green','pink','magenta','lightpink','dodgerblue','darkorange','white','black','aqua','brown','crimson','fuchsia','maroon','lawngreen']; 
		var $filterColor = $('#filter-color');
		var oInput = $filterColor.find('.input');
		var $breviary = $filterColor.find('.breviary');
		var $detail = $filterColor.find('.detail');
		var $more = $filterColor.find('.more');
		var bLi = '';
		var dLi = '';
		for (var i = 0; i < colorArr.length; i++) {
			if(i < 12){
				bLi += '<li data-color="'+colorArr[i]+'" style="background-color:'+colorArr[i]+';border-color:'+(colorArr[i] == 'white' ? '#dfdfe1' : colorArr[i])+';"></li>';
			}
			dLi += '<li data-color="'+colorArr[i]+'" style="background-color:'+colorArr[i]+';border-color:'+(colorArr[i] == 'white' ? '#dfdfe1' : colorArr[i])+';"></li>'
		...
		function setInput(obj){
			var aLi1 = obj.find('li');
			aLi1.each(function(index, el) {
				$(el).on('click',function(){
					oInput.val($(this).data('color'))
					$detail.hide();
				})
			});
		}
	...
	(function(){
		var $report = $('#j-report');
		var $ul = $report.find('.list').find('ul');
		var aLi = $ul.find('li');
		var aLiH = aLi.height();
		if(aLi.size() < 2){
			return;
...
function select(obj){
	...
	var oUl = oSelect.find('ul');
	var oSpan = oSelect.find('.option'); 
	var aLi = oUl.find('a');
	...
	aLi.each(function(i,el){
		$(el).on('click',function(){
			oInput.val($(this).attr('href').substring(1));
